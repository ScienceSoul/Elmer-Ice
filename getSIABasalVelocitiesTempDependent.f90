FUNCTION getSIABasalVelocitiesX (Model, nodenumber, depth) RESULT(Bvelo)
   ...
   TYPE(Variable_t), POINTER :: FreeSurfGrad1, FreeSurfGrad2, TempHomologous
   REAL(KIND=dp), POINTER :: FreeSurfGrad1Values(:), FreeSurfGrad2Values(:), &
                             TempHomologousValues(:)
   INTEGER, POINTER :: FreeSurfGrad1Perm(:), FreeSurfGrad2Perm(:), TempHomologousPerm(:)
   INTEGER :: DIM, i, j
   INTEGER :: NBoundary, NParent, BoundaryElementNode
   INTEGER :: material_id, body_id, other_body_id, bf_id
   REAL (KIND=dp) :: C, Bvelo, scaleFactor, gradh
   REAL (KIND=dp) :: rho, gravity, homoTemp
   REAL (KIND=dp) :: C_b, gamma, exponent_p, exponent_q, ut0
   REAL (KIND=dp) :: tau_b
  ...
  Bvelo = -((C_b*EXP(homoTemp/gamma)) / (rho*gravity)) * (rho*gravity*depth)**(exponent_p-exponent_q) * &
          gradh**(exponent_p-1.0_dp) * FreeSurfGrad1Values(FreeSurfGrad1Perm(nodenumber)) 
...
FUNCTION getSIABasalVelocitiesY (Model, nodenumber, depth) RESULT(Bvelo)
  ...
  Bvelo = -((C_b*EXP(homoTemp/gamma)) / (rho*gravity)) * (rho*gravity*depth)**(exponent_p-exponent_q) * &
          gradh**(exponent_p-1.0_dp) * FreeSurfGrad2Values(FreeSurfGrad2Perm(nodenumber)) 
