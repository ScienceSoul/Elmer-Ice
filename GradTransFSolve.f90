...
!------------------------------------------------------------------------------
     ...
     INTEGER :: dim,n1,n2,i,j,k,l,n,t,iter,NDeg,STDOFs,LocalNodes,NumberOfFaces,istat
...
!------------------------------------------------------------------------------
     ...
     IF ( ASSOCIATED( FSol ) ) THEN
     FPerm   => FSol % Perm
     FValues => FSol % Values
     ELSE
       CALL Fatal( 'GradientSolver:', 'No Gradient variable associated.' )
     END IF
     ...
     ELSE
       CALL Fatal( 'GradientSolver:', 'No Flow Solution associated.' )
      END IF
...
!------------------------------------------------------------------------------
   ...
   IF ( ASSOCIATED( Mesh) ) THEN
     ...
     ELSE  IF ( CoordinateSystemDimension() == 3 ) THEN
        Faces => Mesh % Faces
        NumberOfFaces = Mesh % NumberOfFaces
     END IF
   ELSE
       CALL Fatal( 'GradientSolver:', 'No mesh associated.' )
   END IF
...
! Fab
       IF ( TransientSimulation ) THEN
          IF (AllocationsDone ) DEALLOCATE (PrevF)
       ...
       DO i=1,Solver % NumberOFActiveElements
          ...
          DO COMP=1,9
              IF ( TransientSimulation ) THEN
               PrevF(9*(Indexes(1:n)-1)+COMP) = &
                   FValues(9*(FPerm(NodeIndexes(1:n))-1)+COMP)
            END IF
...
!------------------------------------------------------------------------------
         ...
         F11(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+1 )
         F22(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+2 )
         F12(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+3 )
         F21(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+4 )
         F33(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+5 )
         F13(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+6 )
         F31(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+7 )
         F32(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+8 )
         F23(1:n) = CurrF( 9*(Solver % Variable % Perm(Indexes(1:n))-1)+9 )
...
!------------------------------------------------------------------------------
  DO t=1,NumberOfFaces
        ...
        k = FlowVariable % DOFs
            Velocity = 0.0d0
            DO i=1,k
               Velocity(i,1:n) = FlowValues(k*(FlowPerm(Face % NodeIndexes(1:n))-1)+i)
            END DO
