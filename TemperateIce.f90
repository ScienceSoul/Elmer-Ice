...
! ******************************************************************************
! *
! *  Authors: Thomas Zwinger, Mikko Lyly, Juha Ruokolainen, Hakime Seddik
...
!------------------------------------------------------------------------------
     ...
     TYPE(Variable_t), POINTER :: TempSol,FlowSol,CurrentSol, MeshSol,VarTempHom,VarTempResidual, StressVariable, NormalVar
     ...
     INTEGER, POINTER :: NodeIndexes(:), TempPerm(:),FlowPerm(:),CurrentPerm(:),MeshPerm(:), StressPerm(:), NormalPerm(:)
     ...
     REAL(KIND=dp) :: NonlinearTol, LinearTol, Relax, &
          ...
          HeatCapacityGradient(3), round = 0.0D00, vSn, un
     REAL(KIND=dp), POINTER :: Temp(:), FlowSolution(:), &
          ForceVector(:), PrevSolution(:), HC(:), Hwrk(:,:,:),&
          PointerToResidualVector(:),&
          ResidualVector(:), TempHomologous(:), StressValues(:), NormalValues(:)
     REAL(KIND=dp), ALLOCATABLE :: MASS(:,:), &
       STIFF(:,:), LOAD(:), HeatConductivity(:,:,:), &
         ...
         StiffVector(:), OldValues(:), OldRHS(:), normal(:), velo(:), Sn(:), ut(:), Sig(:,:)
              ...
              IF ( ASSOCIATED( BC ) ) THEN            
                 ...
                 IF (FluxBC) THEN
                    ...
                    IF (isSliding) THEN
                      ...
                      ! Get the normal vector variable
                      !-------------------------------
                      ...
                      DO k=1,n
                        ...
                        DO i=1, DIM
                         DO j= 1, DIM
                            Sig(i,j) = StressValues( 2*DIM *(StressPerm(Element % NodeIndexes(k))-1) + Ind(i,j) )
                         END DO
                        END DO
