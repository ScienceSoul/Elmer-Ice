...
!------------------------------------------------------------------------------
  RECURSIVE SUBROUTINE ThinningFunctionSolver( Model,Solver,dt,Transient )
...
!         LOAD(1:n) = GetReal( BodyForce, 'Source', Found )
     ...
     Norm = DefaultSolve()
      ...
      CALL Info( 'ThinningFunctionSolver', Message, Level=4 )

     ! Average the elemental results to nodal values:
     !-----------------------------------------------
     Var => VariableGet( Mesh % Variables, 'ThinFunction' )
...
!------------------------------------------------------------------------------
       ...
       REAL(KIND=dp) :: LoadAtIp, LGrad(3,3),StrainRate(3,3), Radius, epsi
       REAL(KIND=dp) :: detJ,U,V,W,S,A,L,cu(3),divu,g, M, C0
...
!        L = SUM( LOAD(1:n) *  Basis(1:n) )
         ...
         IF ( CSymmetry ) THEN
           ...
           IF ( Radius > 10*AEPS ) THEN
              StrainRate(3,3) = SUM(Velo(1,1:n) * Basis(1:n) ) / Radius
           END IF
...
!           ---------------
            ...
            ! ----------------
            DO j=1,dim
               A = A - cu(j) * Basis(q) * dBasisdx(p,j)
            END DO
               A = A - divu * Basis(q) * Basis(p)
...
!------------------------------------------------------------------------------
 END SUBROUTINE ThinningFunctionSolver
!------------------------------------------------------------------------------
